...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "PULSE AI: vende y compra software, cursos, templates y eBooks. El sistema operativo de la economía digital latina. 1M+ usuarios, $2.4B procesados. Empieza gratis hoy." },
    ],
...
const testimonials = [
  {
    name: "Carlos Mendoza", role: "Desarrollador de Software", city: "Bogotá", initials: "CM", stars: 5,
    text: "Reemplazé Gumroad + Kajabi + 3 Zapiers en una tarde. PULSE AI gestiona desde la validación de licencias hasta mis correos post-compra. Pasé de $800k/mes a $4.2M en 90 días.",
    revenue: "$4.2M/mes",
  },
  {
    name: "Valentina Torres", role: "Creadora de Cursos Online", city: "Medellín", initials: "VT", stars: 5,
    text: "El programa de afiliados solo pagó la suscripción 10 veces en el primer mes. Los analytics en tiempo real son impresionantes — veo exactamente de dónde viene cada peso.",
    revenue: "$1.8M/mes",
  },
  {
    name: "Andrés Restrepo", role: "Fundador SaaS", city: "Cali", initials: "AR", stars: 5,
    text: "Migramos toda nuestra infraestructura de licencias de software en un fin de semana. La validación por API es sólida y el dashboard le da a los inversionistas exactamente lo que quieren ver.",
    revenue: "$6.1M/mes",
  },
];
...
const aiFeatures = [
  ...
  { icon: BarChart3, title: "AI Analytics Engine", desc: "Detecta churn, identifica upsells, predice LTV y descubre segmentos de alto valor.", metric: "3.2x ROI promedio", color: "text-emerald-400" },
];
    ...
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
              ...
              <div className="p-5 flex gap-5">
                ...
                <div className="flex-1 space-y-4 min-w-0">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {[
                      { l: "NET REVENUE", v: "$8.4M", d: "+23.1%", c: "text-emerald-400" },
                      { l: "MRR", v: "$412k", d: "+18.4%", c: "text-primary" },
                      { l: "CONVERSION", v: "5.62%", d: "+0.8pp", c: "text-blue-400" },
                      { l: "CHURN", v: "1.4%", d: "-0.5pp", c: "text-rose-400" },
                    ].map(m => (
                  ...
                  <div className="rounded-xl bg-white/3 border border-white/6 p-4">
                    <div className="text-xs font-semibold text-white/60 mb-3">Ingresos MRR — Últimos 6 meses</div>
                    <div className="flex items-end gap-2 h-20">
                      {[38, 52, 47, 68, 84, 100].map((h, i) => (
                  ...
                  <div className="rounded-xl bg-primary/8 border border-primary/15 p-3 flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-white/50 leading-relaxed">142 compradores de Neural-Kit no tienen el Masterclass — match 78%. Campaña sugerida: +$42k potencial.</p>
                    </div>
      ...
      <section className="border-y border-border bg-surface/60 py-16 px-6">
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-10">
          <AnimatedStat value={1000000} suffix="+" label="Usuarios activos" />
          <AnimatedStat value={148} suffix="+" label="Productos verificados" />
          <AnimatedStat value={2400} prefix="$" suffix="M" label="En ventas procesadas" />
          <AnimatedStat value={9999} suffix="%" label="Uptime garantizado" />
        </div>
      </section>
            ...
            {timeline.map((step, i) => (
              <div key={step.step} className="relative text-center">
                <div className="relative inline-flex">
                  <div className="size-20 rounded-2xl bg-primary/10 border border-primary/20 flex items-center justify-center mb-6 mx-auto group-hover:bg-primary/20 transition-colors">
      ...
      {/* ── TESTIMONIALS ─────────────────────────────────────────────────── */}
      <section id="testimonios" className="max-w-7xl mx-auto px-6 py-24 border-t border-border">
        <div className="text-center mb-14">
          <div className="text-xs font-mono text-primary uppercase tracking-widest mb-3">Resultados reales</div>
          <h2 className="text-4xl md:text-5xl font-bold tracking-tight">Creadores que ya generan millones</h2>
        </div>
        <div className="grid md:grid-cols-3 gap-6">
          {testimonials.map(t => (
            <div key={t.name} className="group bg-surface border border-border rounded-2xl p-8 hover:border-primary/30 transition-all duration-300 flex flex-col">
              <div className="flex items-center justify-between mb-5">
                ...
                <span className="text-xs font-bold text-primary bg-primary/10 border border-primary/20 px-3 py-1 rounded-full">{t.revenue}</span>
              </div>
              <p className="text-sm text-muted-foreground leading-relaxed mb-6 flex-1">"{t.text}"</p>
              <div className="flex items-center gap-3 pt-4 border-t border-border">
                <div className="size-11 rounded-xl bg-primary/15 border border-primary/20 flex items-center justify-center text-sm font-bold text-primary">{t.initials}</div>
                <div>
                  <div className="font-semibold text-sm">{t.name}</div>
                  <div className="text-[11px] text-muted-foreground">{t.role} · {t.city}</div>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="max-w-5xl mx-auto">
          <div className="grid md:grid-cols-2 gap-10 items-center">
            <div>
              ...
              <p className="text-muted-foreground mb-6 text-lg leading-relaxed">Sin inventario. Sin inversión. Solo comparte tu enlace y cobra automáticamente cada 15 días. Miles de afiliados en Colombia ya ganan más de $2M/mes.</p>
          ...
          <div className="relative rounded-3xl overflow-hidden">
            ...
            <div className="relative px-8 py-16 md:px-16 md:py-20 text-center">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 text-primary text-xs font-mono mb-8">
                <Heart className="size-3.5 fill-primary" /> +1.000.000 creadores confían en PULSE AI
              </div>
